...
(function ($) {
	$.fn.JiSlider = function (options) {
		...
		Animate.prototype = {
			...
			roll: function (time) {
				...
				if (this.controller) {
					...
					this.controller.find('.jislider__button[data-index!=' + this.index + ']').removeClass('jislider__on').css('background-color', '#c8c8c8');
				}
			},
			autoroll: function () {
				if (this.auto)
		...
		li = this.find('ul li');
		var img = this.find('ul li div.bg');
		...
		if (setting.controller) {
			...
			for (var i = 0; i < slides; i++) {
				buttons[i] = $('<div>', {'class': 'jislider__button', 'data-index': (i + 1)}).css({
					border: '0px solid ' + setting.color,
				}).click(function () {
